...
public class IngameGui extends AbstractGui {
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        Nova.getInstance().getModuleManager().get(PostProcessing.class).blurScreen(matrixStack);
        Nova.getInstance().getEventBus().handle(new Render2DEvent(matrixStack, mc.getMainWindow(), partialTicks));
    ...
    private void func_238447_a_(MatrixStack p_238447_1_, ScoreObjective p_238447_2_) {
        ...
        for (Pair<Score, ITextComponent> pair : list1) {
            ...
            String s = TextFormatting.RED + String.valueOf(score1.getScorePoints());
            int k1 = j2 - l * 9;
    ...
    public UUID func_244795_b(ITextComponent textComponent) {
        String s = TextProcessing.func_244782_a(textComponent);
        String s1 = org.apache.commons.lang3.StringUtils.substringBetween(s, "<", ">");
        return s1 == null ? Util.DUMMY_UUID : this.mc.getFilterManager().func_244797_a(s1);
    }

    public void func_238450_a_(ChatType chatType, ITextComponent textComponent, UUID uuid) {
        if (!this.mc.cannotSendChatMessages(uuid) && (!this.mc.gameSettings.field_244794_ae || !this.mc.cannotSendChatMessages(this.func_244795_b(textComponent)))) {
