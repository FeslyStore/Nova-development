package dev.fesly.impl.util.render.textbox;

import com.mojang.blaze3d.matrix.MatrixStack;
import dev.fesly.font.Font;
import dev.fesly.font.GlyphPage;
import dev.fesly.impl.interfaces.game.IMinecraft;
import dev.fesly.impl.util.render.GUIUtil;
...
public class TextBox implements IMinecraft {
    ...
    public void drawString(MatrixStack matrix) {
        ...
        switch (this.textAlign) {
            case CENTER -> {
                ...
                for (int i = 0; i < difference; ++i) {
                    posX = ((posX * (speed - 1)) + (position.x - this.fontRenderer.getStringWidth(this.text.isEmpty() ||
        ...
        /* Renders Text */
        if (this.isEmpty()) {
            this.fontRenderer.drawString(matrix, this.emptyText, posX, position.y, new Color(this.color.getRed(),
                    this.color.getBlue(), this.color.getGreen(), (int) (this.color.getAlpha() * (this.selected ? 0.3F : 0.2F))).hashCode());
        } else {
            this.fontRenderer.drawString(matrix, drawnString.toString(), posX, position.y, this.color.hashCode());
        }
        ...
        final float cursorOffset = this.fontRenderer.getStringWidth(textBeforeCursor.toString());
        ...
        /* Renders cursor */
        if (this.selected) {
            this.fontRenderer.drawString(matrix, "|", (float) (posX + animatedCursorPosition + 1), position.y - 1, new Color(this.color.getRed(), this.color.getBlue(), this.color.getGreen(), this.color.getAlpha() == 0 ? 0 : (int) ((((Math.sin(System.currentTimeMillis() / 150D) + 1) / 2) * 255))).hashCode());
        }
    }
    ...
    public void click(final int mouseX, final int mouseY, final int mouseButton) {
        ...
        this.selected = mouseButton == 0 && GUIUtil.mouseOver(position.x + (textAlign == TextAlign.CENTER ? -width / 2f : 0),
                position.y, width, fontRenderer.getFontHeight(), mouseX, mouseY);
    }
    ...
    public void key(final char typedChar, final int keyCode) {
        ...
        if (InputMappings.isKeyDown(mc.getMainWindow().getHandle(), 29) && keyCode == 47) {
        ...
        } else if (GlyphPage.CHARS.contains(character.toLowerCase())) {
            ...
            if (InputMappings.isKeyDown(mc.getMainWindow().getHandle(), 29)) {
    ...
    private void addText(final String text, final int position) {
        if (fontRenderer.getStringWidth(this.text + text) <= width) {
